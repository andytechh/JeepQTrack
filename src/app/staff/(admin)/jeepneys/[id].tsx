...
export default function AdminJeepneyDetailsScreen() {
  ...
  const openBracketModal = () => {
    if (!jeepney || actionLoading) return;
    setEditBracket(String(jeepney.bracket));
  ...
  const handleEditJeepney = async () => {
    ...
    if (capacity < jeepney.current_occupancy) {
      showError(
        "Invalid Capacity",
        `Capacity cannot be lower than the current occupancy of ${jeepney.current_occupancy}.`,
      );
      return;
    }
    ...
    try {
      ...
      showError(
        "Unable to Update",
        err?.message ?? "The jeepney information could not be updated.",
      );
    } finally {
  ...
  const handleAssignBracket = async () => {
    ...
    const bracket = Number(editBracket.trim());

    if (!Number.isInteger(bracket) || bracket <= 0) {
      showError("Invalid Bracket", "Bracket must be a positive whole number.");
      return;
    }
    ...
    try {
      ...
      setJeepney((current) => (current ? { ...current, bracket } : current));
      ...
      <OceanBackground intensity={0.28}>
        <SafeAreaView className="flex-1">
          <View className="flex-1 px-5 pt-4">
            <Header onBack={handleBack} onEdit={openEditModal} />
    ...
    <OceanBackground intensity={0.28}>
      <SafeAreaView className="flex-1">
        ...
        >
          ...
          <View className="rounded-[25px] border border-white/90 bg-clay-surface p-5">
            <Text className="text-[10px] leading-[16px] text-ink-secondary">
              Edit vehicle information, assign its queue bracket, or manage its
              fleet availability.
            </Text>
        ...
        <AssignBracketModal
          ...
          bracket={editBracket}
          setBracket={setEditBracket}
          saving={actionLoading}
          currentBracket={jeepney?.bracket ?? null}
          onClose={() => !actionLoading && setBracketVisible(false)}
          ...
          <View className="rounded-[30px] border border-white/95 bg-clay-surface p-6">
            <View className="flex-row items-center">
              ...
              <View className="ml-3 flex-1">
                ...
                <Text className="mt-0.5 text-[10px] text-ink-muted">
                  Set this jeepney's queue position bracket.
                </Text>
              </View>
              <Pressable
                disabled={saving}
            ...
            <Text className="mt-2 text-[9px] leading-[15px] text-ink-muted">
              Enter the positive bracket number used by the terminal queue.
            </Text>
            ...
            <View className="mt-6 flex-row">
              ...
              >
                ...
                <Text className="ml-2 text-[11px] font-extrabold text-white">
                  {saving ? "Assigning..." : "Assign Bracket"}
                </Text>
              </Pressable>
            </View>
          </View>
  ...
  return (
    <View className="flex-row items-center">
      ...
      <Pressable
        ...
        className="h-[44px] w-[44px] items-center justify-center rounded-[16px] border border-white/90 bg-clay-surface"
      >
        <Pencil size={19} color={colors.primaryDark} strokeWidth={2.4} />
      </Pressable>
    </View>
  );
}
