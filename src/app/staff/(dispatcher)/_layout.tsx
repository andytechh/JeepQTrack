...
// ─── Main Component ──────────────────────────────────────────────────
// Mirrors (driver)/_layout.tsx exactly: same modal mechanism, same
// shared notificationPanelStore trigger, same realtime subscription
// shape. Only the profile-menu destination route and the "Driver"
// fallback label differ.
export default function DispatcherLayout() {
  const { user, isLoading, logout } = useAuthStore();

  const showNotifications = useNotificationPanel((state) => state.isOpen);
  const closeNotifications = useNotificationPanel((state) => state.close);
              ...
              <View className="flex-row items-center gap-3">
                ...
                <View>
                  ...
                  <Text className="text-gray-400 text-xs">
                    {user?.role || "Dispatcher"} • {user?.email || ""}
                  </Text>
                </View>
              </View>
            </View>
