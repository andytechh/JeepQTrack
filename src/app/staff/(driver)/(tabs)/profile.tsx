...
import { useDriverProfile } from "@/src/shared/hooks/driver/useDriverProfile";
...
export default function DriverProfileScreen() {
  ...
  const handleBack = () => {
    ...
    router.replace("/staff/(driver)/(tabs)");
  };
  ...
  const handleTogglePush = async () => {
    ...
    try {
      ...
      console.error("Driver notification toggle error:", err);
  ...
  const handleSignOut = () => {
    showAlert(
      "Sign out",
      "Are you sure you want to sign out of the Smart Queue driver application?",
      [
        { text: "Cancel", style: "cancel", onPress: () => {} },
        {
          ...
          onPress: async () => {
            ...
            // won't redirect back into the driver stack.
            logout();
            ...
            try {
              ...
              if (signOutError) {
                console.error("Driver sign out error:", signOutError);
              }
            } catch (err) {
              console.error("Driver sign out error:", err);
            } finally {
              setSigningOut(false);
            }
          },
          ...
          <View className="flex-1 px-5 pt-4">
            ...
            <View className="mt-8 items-center rounded-[28px] border border-white/90 bg-clay-surface px-6 py-9 shadow-clay">
              ...
              <Text className="mt-2 text-center text-[11px] leading-[17px] text-ink-secondary">
                {error ?? "Your driver profile could not be loaded."}
              </Text>
        ...
        >
          ...
          <View className="mt-5 items-center rounded-[27px] border border-white/90 bg-clay-surface p-6 shadow-clay">
            ...
            <View className="mt-3 flex-row items-center rounded-full bg-ocean-100 px-3 py-1.5">
              <Car size={12} color={colors.primaryDark} strokeWidth={2.4} />
          ...
          {!editing && (
            <>
              <SectionTitle
                icon={
                  <Car size={17} color={colors.primaryDark} strokeWidth={2.3} />
                }
              ...
              {profile.jeepney ? (
                <View className="overflow-hidden rounded-[25px] border border-white/90 bg-clay-surface shadow-clay">
                  <ProfileRow
                    icon={IdCard}
                    label="Plate Number"
                    value={profile.jeepney.plate_number}
                  />
                  {profile.jeepney.jeep_name && (
                    <>
                      <Divider />
                      <ProfileRow
                        icon={Car}
                        label="Vehicle Name"
                        value={profile.jeepney.jeep_name}
                      />
                    </>
                  )}
                  <Divider />
                  <ProfileRow
                    icon={MapPin}
                    label="Terminal & Bracket"
                    value={[
                      profile.jeepney.terminal_id != null
                        ? `Terminal ${profile.jeepney.terminal_id}`
                        : null,
                      `Bracket ${profile.jeepney.bracket}`,
                    ]
                      .filter(Boolean)
                      .join(" • ")}
                  />
                  ...
                  />
                  {profile.jeepney.capacity != null && (
                    <>
                      <Divider />
                      <ProfileRow
                        icon={Smartphone}
                        label="Capacity"
                        value={`${profile.jeepney.capacity} passengers`}
                      />
                    </>
                  )}
                </View>
              ) : (
                <View className="items-center rounded-[25px] border border-white/90 bg-clay-surface px-6 py-7 shadow-clay">
                  ...
                  <Text className="mt-3 text-center text-[12px] leading-[18px] text-ink-secondary">
                    No vehicle is assigned to your account yet. Contact your
                    dispatcher or system administrator to get assigned to a
                    jeepney.
                  </Text>
                </View>
              )}
          ...
          <View className="overflow-hidden rounded-[25px] border border-white/90 bg-clay-surface shadow-clay">
            <SettingsActionRow
              ...
              onPress={() => {
                showAlert(
                  "Security",
                  "Your Smart Queue driver account is authenticated through Supabase. For password or account security changes, contact the system administrator.",
                );
              }}
            />
